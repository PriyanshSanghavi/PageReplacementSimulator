...
public class LifoSimulatorFragment extends Fragment { //Space Complexity:O(ref_len*frames) //Time Complexity:O(ref_len * frames)
    EditText nof,rs,lors;
    Button lifo_simulator;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_lifo_simulator, container, false); //Space Complexity:O(ref_len*frames) //Time Complexity:O(ref_len * frames)
        nof = v.findViewById(R.id.nof);
        rs = v.findViewById(R.id.rs);
        lors = v.findViewById(R.id.lors);
        lifo_simulator = v.findViewById(R.id.lifo_simulator);
        ...
        lifo_simulator.setText("SIMULATOR");
        lifo_simulator.setOnClickListener(new View.OnClickListener() {
            @RequiresApi(api = Build.VERSION_CODES.N)
            @Override
            public void onClick(View view) {
                if (lifo_simulator.getText() == "SIMULATOR") {
                    if (validate()) {
                        ...
                        for (int i = 0; i < ref_len; i++) {
                            ...
                            if (search == -1) // if page is not inside main memory
                            {
                                ...
                                //Pointer is a variable used to change the index in the queue buffer. It signifies which page is to be replaced according to LIFO algorithm
                                if (pointer == frames) {
                        ...
                        lifo_simulator.setText("RESET");
                    }
                }
                    else if (lifo_simulator.getText() == "RESET") {
                        frames = 0;
                        pointer = 0;
                        hit = 0;
                        fault = 0;
